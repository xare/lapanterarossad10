...
class GeslibApiLines {
	...
	public function storeToLines(){
		...
		$filename = $this->drupal->getLogLoggedFile();
		...
		foreach ( $lines as $line ) {
			...
			\Drupal::queue('geslib_store_geslib_lines')->createItem($item);
		}
	 ...
	 * @return void
	 */
	public function readLine( string $line, int $log_id ){
		var_dump($line);
		$data = explode( '|', $line ) ;
		array_pop($data);
	...
	private function process1L( array $data, int $log_id ) {
		...
		$content_array = array_combine($keys, $data);
		$content_array = $this->geslibApiSanitize->sanitize_content_array($content_array);
		$this->drupal->insertData( $content_array, $data[1], $log_id , 'editorial');
	}
	...
	private function process3( array $data, int $log_id ) {
		...
		$content_array = array_combine( $keys, $data );
		$content_array = $this->geslibApiSanitize->sanitize_content_array( $content_array );
		$this->drupal->insertData( $content_array, $data[1], $log_id , 'product_cat');
	}
	 ...
	 * @return void
	 */
	private function process5( array $data, int $log_id ) {
	 ...
	 * @return void
	 */
	private function processAUT( array $data, int $log_id ) {
		...
		$this->drupal->insertData( $content_array, $data[1], $log_id , 'autor');
	}
	 ...
	 * @return void
	 */
	private function processAUTBIO( array $data, int $log_id ) {
